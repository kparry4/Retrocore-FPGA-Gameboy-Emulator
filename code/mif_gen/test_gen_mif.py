...
def main():
    ...
    rom32_16 = [r for r in range(rom32_16_dim)]
...
#               16 kB ROM BANKS
##########################################################
    with open(args.rom32_16_path, "w") as fout:
        addr_counter = count(0)
        data_lines = [
            "{:04x} : {:04x};".format(next(addr_counter), e)
            for e in rom32_16]
        fout.write(
            header_template.format(depth=rom32_16_dim, width=16) +
            "\n".join(data_lines) +
            footer_template)
