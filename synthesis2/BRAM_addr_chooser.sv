    ...
    ***/
    ...
    always_comb begin
        if(doing_dma) begin
                ...
                rom0_addr  =  (14)'( convert_to_BRAM_addr(dma_src_addr, `ROM_0_START));
                vram_addr_rw = (12)'(convert_to_BRAM_addr(dma_src_addr, `VRAM_START));
                vram_addr_r = 12'h0000;
                exram_addr_r = (12)'(convert_to_BRAM_addr(dma_src_addr, `EXRAM_START));
                wram_addr_r  = (12)' (convert_to_BRAM_addr(dma_src_addr, `WRAM_START));

                //alow dma to COPY to oam table
                oam_addr_rw = (7)'(convert_to_BRAM_addr(dma_dest_addr, `OAM_START)); //allow dma
                oam_addr_r = 7'h00;

                hram_addr_r = (7)'(convert_to_BRAM_addr(cpu_addr_read, `HRAM_START)); //hram is LUTS, not bram

            end else begin

                //if NO dma, address is offset from CPU's input addr
                rom0_addr    = (14)'(convert_to_BRAM_addr(cpu_addr_read,  `ROM_0_START));
                exram_addr_r = (12)'(convert_to_BRAM_addr(cpu_addr_read, `EXRAM_START));
                wram_addr_r  = (12)'(convert_to_BRAM_addr(cpu_addr_read, `WRAM_START));
                hram_addr_r  = (7)'(convert_to_BRAM_addr(cpu_addr_read, `HRAM_START));  //hram is LUTS, not bram
                ...
                if(ppu_mode == 0 || ppu_mode == 1 || ppu_mode == 2) begin
                    ...
                    if(ppu_mode == 2 && LCDC_R[7]) begin
                        ...
                        oam_addr_rw = (7)'(convert_to_BRAM_addr(ppu_addr1, `OAM_START));
                        oam_addr_r =  (7)'(convert_to_BRAM_addr(ppu_addr2, `OAM_START));
                        oam_wren  = 1'b0;

                    end else begin
                        oam_addr_rw = (7)'(convert_to_BRAM_addr(cpu_addr_write, `OAM_START));
                        oam_addr_r =  (7)'(convert_to_BRAM_addr(cpu_addr_read,  `OAM_START));
                        //cpu can write
                        oam_wren  = cpu_wren && within_range(cpu_addr_write, `OAM_START, `OAM_END);
                    end

                    vram_addr_rw = (12)'(convert_to_BRAM_addr(cpu_addr_write, `VRAM_START));
                    vram_addr_r =  (12)'(convert_to_BRAM_addr(cpu_addr_read,  `VRAM_START));
                    //cpu can write
                    vram_wren  = cpu_wren && within_range(cpu_addr_write, `VRAM_START, `VRAM_END);

                end else begin
                        //in DRAWING MODE
                        vram_addr_rw = (12)'(convert_to_BRAM_addr(ppu_addr1, `VRAM_START));
                        vram_addr_r = (12)'(convert_to_BRAM_addr(ppu_addr2, `VRAM_START));
